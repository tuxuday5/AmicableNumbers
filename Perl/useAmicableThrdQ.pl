...
sub Main($) {
  ...
  @Threads = LaunchThreads $SequenceSplits,$Args->Threads,$Queue,$NumCpus ;
  
  DeQueueNos $Queue,$Args->Threads,%Pairs;
...
sub LaunchThreads($$$$) {
  my ($SequenceList,$TotalThreads,$Queue,$NumCpus) = @_ ;
  my (@Threads,$Core,$Thread) ;

  $Core   = $NumCpus-1;
  foreach my $SeqRange (@$SequenceList) {
    $Thread = threads->create(
      'FindAmicable',$SeqRange->[0],$SeqRange->[1],
      $Queue,$Core
    );
...
sub FindAmicable($$$$) {
  ...
  foreach my $Number ($StartNo..$EndNo) {
    ...
    if($Obj->IsAmicable) {
      $Queue->enqueue([$Tid,$Number,$Obj->MyPair]);
      $TotalPairs++;
    }
  ...
  $Queue->enqueue([$Tid,0,0]);
  printf("[$Tid] took %.2f(s) for range $StartNo-$EndNo exiting\n",Time::HiRes::tv_interval(\@TimeStart));
  return int($TotalPairs)/2; #as we find amicable for both nos in a pair
}
__END__
